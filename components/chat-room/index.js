  ...
  const getMyself = () => {
    for (var x = 0; x < props.RoomUsers.length; x++) {
      if (props.RoomUsers[x].uid === props.Auth().currentUser.uid) {
        return (
          <View
            style={{...styles.userContainer, marginLeft: 6}}
            ...
            <Text style={{...styles.distance, color: '#434187'}}>{'N/A'}</Text>
          </View>
        );
      }
    }
              ...
              {props.RoomUsers.map((user, index) =>
                user.uid !== props.Auth().currentUser.uid ? (
                  <View style={styles.userContainer} key={index}>
                    ...
                    {user.isTyping ? (
                      <Text style={styles.userName}>is typing</Text>
                    ) : null}
                  </View>
                ) : null,
              )}
              ...
              <View
                ...
                {item.data.author.uid === props.Auth().currentUser.uid ? (
                  <>
                    ...
                    <Image
                      source={{uri: item.data.author.photoURL}}
                      style={{...styles.chatAvatar, marginLeft: 12}}
                    />
                  </>
                ) : (
                  <>
                    <Image
                      source={{uri: item.data.author.photoURL}}
                      style={{...styles.chatAvatar, marginRight: 12}}
                    />
