  ...
  const generateCode = () => {
    ...
    while (generatedCode.length < 4) {
      let randomNumber = Math.round(
        0 + Math.random() * (letters.length - 1 - 0),
      );
    ...
    <KeyboardAvoidingView
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        <View style={styles.innerContainer}>
          <Image
            source={require('./images/JoinOrCreateImage.png')}
            style={styles.image}
          />
          <TouchableOpacity
            style={styles.backButton}
            onPress={() => props.updateScreen('greeting')}>
          ...
          <View style={styles.textInputRow}>
            ...
            <TouchableOpacity
              ...
              <Icon
                name="chevron-right"
                size={24}
                color={enteredCode.length === 4 ? '#434187' : '#515151'}
              />
            </TouchableOpacity>
          </View>
