...
const ConfigureProfile = (props) => {
  ...
  const getRandomPhoto = (searchTerm) => {
    let pic = '';
    fetch(
      'https://en.wikipedia.org/w/api.php?action=query&list=allimages&aiprop=user|timestamp|url|canonicaltitle&aisort=name&aiprefix=' +
        searchTerm +
        '&origin=*&format=json',
    )
      ...
      <KeyboardAvoidingView
        ...
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <View style={styles.innerContainer}>
            <Image
              source={require('./images/NameImage.png')}
    ...
    return (
      <View
        ...
        <Text style={styles.headline}>
          {'Nice to meet you, ' +
            userName +
            '. We chose a profile picture for you.'}
        </Text>
        <Image
          source={
            photoURL
              ? {uri: photoURL}
              : require('./images/PlaceholderAvatar.png')
          }
          style={styles.avatar}
        />
        <View style={{flex: 1}} />
        <TouchableOpacity
          style={{...styles.primaryButton, marginRight: 0, alignSelf: 'center'}}
          onPress={() => props.updateScreen('join-or-create')}>
