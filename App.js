...
const App: () => React$Node = () => {
        ...
        if (!granted) {
          granted = await PermissionsAndroid.request(
            PermissionsAndroid.PERMISSIONS.ACCESS_FINE_LOCATION,
            {
              ...
              message:
                'TrashTalk needs access to your location so your friends can see how far away you are.',
    ...
    auth()
      ...
      .catch((err) => {
        if (err.code === 'auth/email-already-in-use') {
          //show error screen
          setError({
            errorTitle: 'You shall not pass.',
            errorDescription:
              'Someone has already created an account with this email address. Was it you?',
            returnScreen: 'registration',
          });
          ...
          setError({
            errorTitle: 'You shall not pass.',
            errorDescription:
              "Whoops. Looks like the email address you provided wasn't valid. Do you want to try again?",
            returnScreen: 'registration',
          });
          ...
          setError({
            errorTitle: 'You shall not pass.',
            errorDescription:
              "Sorry, but we're not accepting new accounts at this time. Please try again later!",
            returnScreen: 'greeting',
          });
        } else if (err.code === 'auth/weak-password') {
          setError({
            errorTitle: 'You shall not pass.',
            errorDescription:
              "Your password isn't strong enough. Please try a new password that uses mixed case letters, numbers, and symbols.",
            returnScreen: 'registration',
          });
        }
      ...
      .catch((err) => {
        if (
          ...
          setError({
            errorTitle: 'You shall not pass.',
            errorDescription:
              "The email address or password you've entered don't match our records. Would you like to try again?",
            returnScreen: 'login',
          });
  ...
  return (
    <>
      ...
      <SafeAreaView>
        {currentScreen === 'error' ? (
          <LoginError ErrorDetails={error} updateScreen={updateScreen} />
        ) : null}
        ...
        {currentScreen === 'registration' ? (
          <Registration updateScreen={updateScreen} doRegister={doRegister} />
        ) : null}
        {currentScreen === 'configure-profile' ? (
          <ConfigureProfile Auth={auth} updateScreen={updateScreen} />
        ) : null}
        {currentScreen === 'login' ? (
          <Login doSignIn={doSignIn} updateScreen={updateScreen} />
        ) : null}
        {currentScreen === 'join-or-create' ? (
          <CreateOrJoin updateScreen={updateScreen} joinRoom={joinRoom} />
        ) : null}
